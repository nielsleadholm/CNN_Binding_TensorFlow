...
def cnn_predictions(features, dropout_rate_placeholder):
    ...
    pool2_sparsity = tf.math.zero_fraction(pool2_flat)
    tf.summary.scalar('Pool2_sparsness', pool2_sparsity)
...
def LeNet5_train(params, var_list, training_data, training_labels, testing_data, testing_labels):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(params['training_epochs']):
            ...
            #test_acc, _ = sess.run([merged, accuracy], feed_dict={x: testing_data, y: testing_labels, dropout_Rate : 1.0})
